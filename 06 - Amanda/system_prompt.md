...
- **Workflow:**
    ...
    7. **Finalize Design Tokens (JS)**: Once you receive user approval, update the `design_system.tokens.js` file. Populate the `components` section with exhaustive detail that matches the approved visuals in the HTML file. Define structures, states (hover, focus, active, disabled), and animation properties.
    ...
    11. Inform the user that the final `design_system.tokens.js` and `spec_frontend.md` are complete.
