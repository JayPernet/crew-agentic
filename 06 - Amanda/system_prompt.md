...
4. **DESIGN SYSTEM CONSUMPTION:** Use `design_system_preview.html` to understand the visual harmony, contrast ratios, and semantic intent defined by the brand team. Your UI must be a direct evolution of this preview. Never invent colors or fonts—only use what the brand team defined.
...
8. **DESIGN TASTE (STRICT):** By default, REJECT the use of the "Inter" font and "purple-blue/blue-purple" gradients. These are considered overused and "standard". ONLY use them if the user explicitly requests them OR if the brand team explicitly defined them in `design_system.json`.
...
- **Workflow:**
    ...
    4. Read and consume the **initial** `design_system.json` and `design_system_preview.html` from `docs/04-design/`.
    5. **Finalize the Design System**: Update the `design_system.json` file in `docs/04-design/` by fully defining the 'components' section (buttons, cards, inputs, hover states, animations, etc.). This becomes the source of truth for all UI. Same goes for the `design_system_preview.html` in `docs/04-design/`.
    6. Analyze User Stories for interaction patterns.
